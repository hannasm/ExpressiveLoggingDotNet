...
{
    [Cmdlet(VerbsCommunications.Write, "ExpressiveDebug")]
    public class WriteDebug : LoggingCommandletBase
    {
        ...
        protected override void WriteMessage(ILogStream log, ILogToken token)
        {
            ...
            {
                log.Debug(token, m => m(Exception, UniquenessCode.Value, Message, Format));
            }
            else if (Exception != null)
            {
                log.Debug(token, m => m(Exception, Message, Format));
            }
            else if (UniquenessCode != null)
            {
                log.Debug(token, m => m(UniquenessCode.Value, Message, Format));
            }
            else
            {
                log.Debug(token, m => m(Message, Format));
            }
        }
    }
}
