...
publish $version (join-path $solutionDir ('ExpressiveLogging.' + $version + '.nupkg'));
publish $version (join-path $solutionDir ('ExpressiveLogging.' + $version + '.symbols.nupkg'));
publish $version (join-path $solutionDir ('ExpressiveLogging.PowershellV5Logging.' + $version + '.nupkg'));
publish $version (join-path $solutionDir ('ExpressiveLogging.PowershellV5Logging.' + $version + '.symbols.nupkg'));